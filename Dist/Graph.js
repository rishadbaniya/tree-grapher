    ...
    GetLayout = (direction = "leftToRight", nodeGroupFilter = (group) => true) => {
        ...
        const layout = new FlexTreeLayout({
            children: group => {
                ...
                if (globalThis.DEV)
                    Assert(children_noSelfSideBoxes_addChildSideBoxes.every(a => !a.IsDestroyed()), "layout.children encountered a destroyed node-group!");
                return children_noSelfSideBoxes_addChildSideBoxes;
            },
            nodeSize: node => {
                const data = node.data;
                if (globalThis.DEV)
                    Assert(!data.IsDestroyed(), "layout.nodeSize encountered a destroyed node-group!");
            ...
            spacing: (nodeA, nodeB) => {
                if (globalThis.DEV)
                    Assert(!nodeA.data.IsDestroyed() && !nodeB.data.IsDestroyed(), `layout.nodeSize encountered a destroyed node-group! @nodeADestroyed:${nodeA.data.IsDestroyed()} @nodeBDestroyed:${nodeB.data.IsDestroyed()}`);
